...
//	const size_t num_buckets = size_t(1) << log_num_buckets;
	...
	if(true) {
		...
		typedef RamSort<phase1::entry_1, phase1::get_y<phase1::entry_1>> RamSort1;
		
		RamSort1 sort(test_bits, log_num_buckets, "test");
	...
	if(false) {
		...
		typedef RamSort<phase1::entry_4, phase1::get_y<phase1::entry_4>> RamSort4;
		
		RamSort4 sort(test_bits, log_num_buckets, "test");
